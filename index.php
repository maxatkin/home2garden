...
$yearnow = date("Y");
$banner = "";
if ($yearnow == "2009")$banner = "All items discounted 20% during January.";
...
global $var_column;
 $content = array(); 
  ...
  if($condition == "") $ret = ReadDB($table, array("id", "name", "description"), "", array("displayorder", "ASC"), array("priceid", ""));
  else $ret = ReadDB($table, array("id", "name", "description"), array($condition, (integer)$condition_id), array("displayorder"), "ASC", array("priceid", ""));
  if($ret["status"] == "Error") return $ret;
  $category_icon = "<table id='notfull' border = '0' ><tr>";
  ...
  foreach($category_list as $category => $category_info){
	...
	$imagepath2 = "images/" . $table . "/" . $id . "/" . $id . ".jpg";
	//name is added to the qs:
    $imagelink = "index.php?" . $link . $id."&name=".$content["name"];
	...
	if($firstloop == True){
	$itempicture = "<img src='". URL_PATH. $imagepath2 ."'/>";
	$firstloop = False;
    }
	$content["price"]= $category_info["displayorder"];
    $ret = SendPage("category.tem", $content, FALSE);
    if($ret["status"] == "Error") return $ret;
	if ($var_column == 2){
    $category_icon = $category_icon . "<td  valign=\"top\" style='text-align: center; width: 300px;'>" . $ret["value"] . "</td>";
	}
	else{
	$category_icon = $category_icon . "<td  valign=\"top\" style='text-align: center; width: 135px;'>" . $ret["value"] . "</td>";
    }
    $column_count = $column_count + 1;
    if($column_count == $var_column){
      $category_icon = $category_icon . "</tr></table><table id='notfull' border = '0' ><tr>";
      $column_count = 0;
    }
  }
  ...
  if($subcatinfo["status"] != "Error"){
    ...
    if($catinfo["status"] != "Error"){
      ...
      $subcatlink = URL_PATH . "index.php?" . "subcat=" . $subcatinfo[0]["id"];
		 $location = $subcatinfo[0]["name"] ; 
		 print $catinfo[0]["name"];
		 print $subcatinfo[0]["name"];
    }
  }
...
if($var_column == 4){
  ...
  $textbox_content["item_image"] = $itempicture ;
